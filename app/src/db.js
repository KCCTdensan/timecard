...
class Sqlite {
  constructor(dbFilePath) {
    ...
    try {
      ...
      this.db.serialize(() => {
        this.db.run(
          `CREATE TABLE IF NOT EXISTS ${userTable}(id INTEGER UNIQUE, name TEXT, course TEXT)`
        )
        this.db.run(
          `CREATE TABLE IF NOT EXISTS ${statusTable}(d_id TEXT UNIQUE, prev_d_id TEXT, id INTEGER, updated TEXT, in_room INTEGER)`
        )
      })
    } catch (err) {
      throw err
  ...
  async getUser({ id }) {
    ...
    if (row) {
      const user = new classes.user({
        ...
        course: Buffer.from(row.course, "base64").toString("utf-8"),
      })
      return user.setStatus(await this.getLatestStatus({ id }))
    } else {
      return null
    }
  }
  ...
  async updateUser(user) {
    ...
    await this.run(
      `INSERT OR REPLACE INTO ${userTable}(${keys.join(
        ","
      )}) VALUES($${keys.join(",$")})`,
      values
    )
  ...
  async getLatestStatus({ id }) {
    ...
    const row = await this.get(
      `SELECT * FROM ${statusTable} WHERE id=? ORDER BY updated DESC`,
      id
    )
    if (row) {
  ...
  async updateUserStatus(user) {
    ...
    if (!user.status.updated) {
      ...
      keys.push("updated")
      values["$updated"] = Math.round(user.status.updated.getTime() / 1000)
    }
    ...
    while (true) {
      const hex = rndHex(6)
      if (
        !(await this.get(`SELECT d_id FROM ${statusTable} WHERE d_id=?`, hex))
      ) {
        values["$d_id"] = hex
        break
      }
    ...
    const prevRow = await this.get(
      `SELECT * FROM ${statusTable} WHERE id=?`,
      user.id
    )
    if (prevRow) {
      keys.push("prev_d_id")
    ...
    await this.run(
      `INSERT OR REPLACE INTO ${statusTable}(${keys.join(
        ","
      )}) VALUES($${keys.join(",$")})`,
      values
    )
